...
parser.add_argument(
    "--batch_size", type=int, default=10000, help="Batch size for mini-batch training."
)
parser.add_argument(
    "--weight_decay",
    type=float,
...
parser.add_argument(
    "--lr_decay", type=float, default=1.0, help="Lr decay hyperparameter ."
)
...
parser.add_argument("--train", action="store_true", help="Train on/off")
parser.add_argument(
    "--tdata", type=str, default=None, help="Name of the training dataset"
)
parser.add_argument(
    "--vdata", type=str, default=None, help="Name of the validation dataset"
)
parser.add_argument(
    "--test_data", type=str, default=None, help="Name of the test dataset"
)
parser.add_argument(
    "--vlabels", type=str, default=None, help="Labels of the test dataset in Figure"
)
parser.add_argument(
    "--save_model", type=str, default=None, help="Name of saved model parameters"
)
parser.add_argument(
    "--pre_save_model",
    type=str,
...
args.tdata = [ data for data in args.tdata]
...
if args.train:
    for model_ind in range(k_fold):
        fns = []
        train_dataset = get_dataset_dataloader(
            args.tdata, batch_size=args.batch_size, num_workers=8, train=True
        )
        if args.vdata is not None:
            val_dataset = get_dataset_dataloader(
                args.vdata, batch_size=args.batch_size, num_workers=8, train=True
            )
...
if dataset is None:
    dataset = get_dataset_dataloader(
        args.tdata, batch_size=args.batch_size, num_workers=16, train=True
    )
...
if True:
    ...
    for test_fname in args.test_data:
        _, unlabel_xs, _ = get_dataset_dataloader(
            test_fname, batch_size=args.batch_size, num_workers=8
        )

        likeness_list = (
            score(trainer, unlabel_xs, device).cpu().detach().numpy().reshape(-1)
        )
        unlab_list.append(likeness_list)

    colors = ["y", "r", "lime", "violet", "g", "orange", "b", "black", "grey"][
        : len(args.vlabels)
    ]
    ...
    if args.vdata is not None:
        _ ,val_xs, _ = get_dataset_dataloader(
            args.vdata, batch_size=args.batch_size, num_workers=8, train=True
        )

        likeness_list = (
            score(trainer, val_xs, device).cpu().detach().numpy().reshape(-1)
        )

        mean, std = obtain_mean_variance(likeness_list)
        print("Val mean, std :", mean, std)
